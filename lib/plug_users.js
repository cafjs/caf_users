...
exports.newInstance = async function($, spec) {
    try {
        ...
        const offersSetPrefix = json_rpc.joinName(OFFERS_SET_PREFIX, appName) +
                ':';
        const acceptsSetPrefix = json_rpc.joinName(ACCEPTS_SET_PREFIX, appName) +
                ':';
        ...
        that.registerApp = function(app, cost) {
            ...
            const args = [app, cost || defaultTimePerUnit, appPublishCost, user];
            ...
            return new Promise(async function (resolve, reject) {
                try {
                    resolve(arrayToObject(
                        await that.describeReputation(user),
        ...
        that.transferUnits = function(nonce, from, to, units, id) {
            const expires = defaultHoldTimeInSec * 1000 + (new Date()).getTime();
        ...
        that.getUserInfo = async function(user) {
            try {
                ...
                const cas = arrayToObject(await that.listCAs(user),
                                        x => parseFloat(x));
                const offers = arrayToObject(await that.listOffers(user),
                                           x => decodeTransferValues(x));
                offers = removeKeyPrefix(offers, transferPrefix);
                const accepts = arrayToObject(await that.listAccepts(user),
                                            x => decodeTransferValues(x));
