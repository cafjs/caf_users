 ...
 *        appRegisterCost: number,  accountsApp: string,
 ...
 *  * `appWriterFraction`:  default fraction of a unit given to the app writer
 * for each new registered CA in that app.
 *  * `accountsApp`: The name of the accounts app, e.g., `root-accounts`.
 *  * `appPublishCost`:  initial number of units needed to register a new app.
 *  * `appRegisterCost`:  cost in days per unit to keep an app registered.
...
const APP_REGISTER_MAP = USER_PREFIX + 'appsRegister';
const APP_PLANS_MAP = USER_PREFIX + 'appsPlans';
const APP_PROFIT_MAP = USER_PREFIX + 'appsProfit';
...
exports.newInstance = async function($, spec) {
    try {
        ...
        const appRegisterMap = json_rpc.joinName(APP_REGISTER_MAP, appName);
        const appPlansMap = json_rpc.joinName(APP_PLANS_MAP, appName);
        const appProfitMap = json_rpc.joinName(APP_PROFIT_MAP, appName);
        ...
        assert.equal(typeof(spec.env.appRegisterCost), 'number',
                     "'spec.env.appRegisterCost' is not a number");
        const appRegisterCost = spec.env.appRegisterCost;
        ...
        that.registerApp = function(app, plan, profit, days) {
            const user = json_rpc.splitName(app)[0];
            const expireTime = (new Date()).getTime()/(24*60*60*1000.0) +
                  appRegisterCost;
            const args = [app, days || defaultTimePerUnit, appPublishCost,
                          user, expireTime, plan, profit];
            return doLuaAsync('registerApp',
                              [appsSetPrefix + user, appMap, userMap, statsMap,
                               unitsStream, appRegisterMap, appPlansMap,
                               appProfitMap], args);
        };

        that.unregisterApp = function(app) {
            const user = json_rpc.splitName(app)[0];
            return doLuaAsync('unregisterApp', [appMap, appsSetPrefix + user,
                                                appStatsListPrefix + app,
                                                appRegisterMap, appPlansMap,
                                                appProfitMap], [app]);
        };
        ...
        that.registerCA = function(ca) {
            ...
            const regExpireTime = time + appRegisterCost;
            return doLuaAsync('registerCA', [
                userMap, appMap, casMap, casSetPrefix + user, statsMap,
                unitsStream, appRegisterMap, appProfitMap
        ...
        that.listAppsRegistration = (user) =>
            doLuaAsync('listAppsRegistration', [appsSetPrefix + user,
                                                appRegisterMap], []);
        that.listAppsPlans = (user) =>
            doLuaAsync('listAppsPlans', [appsSetPrefix + user, appPlansMap],
                       []);
        that.listAppsProfit = (user) =>
            doLuaAsync('listAppsProfit', [appsSetPrefix + user, appProfitMap],
                       []);
        that.listCAs = (user) =>
        ...
        const mergeAppsInfo = function(appsCost, appsRegistration, appsPlans,
                                       appsProfit) {
            const result = {};
            Object.keys(appsCost).forEach((x) => {
                result[x] = {cost: appsCost[x]};
                if (appsRegistration[x]) {
                    result[x].expires = appsRegistration[x];
                }
                if (appsPlans[x]) {
                    result[x].plan = appsPlans[x];
                }
                if (typeof appsProfit[x] === 'number') {
                    result[x].profit = appsProfit[x];
                }
            });
            return result;
        };
        ...
        that.getUserInfo = async function(user) {
            try {
                ...
                const appsCost = arrayToObject(await that.listApps(user),
                                               x => parseFloat(x));
                const appsRegistration = arrayToObject(
                    await that.listAppsRegistration(user), x => parseFloat(x)
                );
                const appsPlans = arrayToObject(
                    await that.listAppsPlans(user), x => x
                );
                const appsProfit = arrayToObject(
                    await that.listAppsProfit(user), x => parseFloat(x)
                );
                const apps = mergeAppsInfo(appsCost, appsRegistration,
                                           appsPlans, appsProfit);
                ...
                return [null, {
                    user: userInfo, apps: apps, cas: cas,
                    offers: offers, accepts: accepts, reputation: reputation
                }];
            } catch (err) {
                return [err];
            }
        ...
        for (let i=0; i<Math.floor(preRegisterApp.length/2); i++) {
            if (preRegisterApp[2*i] && preRegisterApp[2*i+1]) {
                ...
                await that.registerApp(preApp, DEFAULT_PLAN, 0, DEFAULT_DAYS);
            }
        }
